...
- **BodyModel.scn mesh names are load-bearing** — `Muscle.swift` maps 58 exact trainable regions. Forty-eight visualized owners cover 120 left/right SceneKit nodes (60 mesh bases), while ten regions are explicitly unvisualized. Two posterior-serratus mesh bases remain explicit non-trainable scene surfaces. Renaming meshes or enum cases breaks highlighting silently; `MuscleMappingTests` guards this.
