...
`vivobody/Resources/catalog.json`. The runtime catalog contains exactly the 82
active families and their 196 reviewed exercises; the synthetic fixture and
...
Equipment identifies the implement geometry that materially defines the
...
- The projection contains exactly 82 family IDs and 196 exercise records; it is
  stable under family file discovery order and excludes the synthetic fixture
  and supplemental `--family` validation inputs.
- The evidence registry contains exactly 219 source identities, each referenced
